...
struct RowNavigationLink<Label: View>: View {

    private var label: (() -> Label)?
    private var title: String?
    private var value: AnyHashable

    @Environment(\.pushView) private var pushView: (AnyHashable) -> ()
    var body: some View {
        button()
            .buttonStyle(.plain)
    }
    ...
    public init<P>(value: P, @ViewBuilder label: @escaping () -> Label) where P : Hashable {
        self.label = label
        self.value = value
    }

    public init<P>(_ title: String, value: P) where Label == Text, P : Hashable {
        self.title = title
        self.value = value
    }
